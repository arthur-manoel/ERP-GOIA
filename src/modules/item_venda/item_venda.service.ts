import {
  ...
  type ItemVenda,
} from './item_venda.repository.js'
import type {
  CreateItemVendaInput,
  ListItemVendaInput,
  UpdateItemVendaInput,
} from './item_venda.schema.js'

export const ITEM_VENDA_MESSAGES = {
  NOT_FOUND: 'Item da venda não encontrado',
  ...
  INVALID_FOREIGN_KEY: 'Venda ou produto informado não existe',
  DUPLICATE_RECORD: 'Já existe um item da venda com os dados informados',
  DELETED: 'Item da venda excluído com sucesso',
} as const

export class NotFoundError extends Error {
  constructor(message = ITEM_VENDA_MESSAGES.NOT_FOUND) {
    super(message); this.name = 'NotFoundError'
  }
}
export class ConflictError extends Error {
  constructor(message = ITEM_VENDA_MESSAGES.DUPLICATE_RECORD) {
...
export class ItemVendaService {
  public async criar(data: CreateItemVendaInput): Promise<ItemVenda> {
  ...
  public async listar(filters: ListItemVendaInput) {
    const page = filters.page ?? 1
    const limit = filters.limit ?? 20
    const result = await findAll(
      { idVenda: filters.id_venda, idProduto: filters.id_produto },
      { page, limit }
    )
    return {
      rows: result.rows,
  ...
  public async buscarPorId(id: number): Promise<ItemVenda> {
    const item = await findById(id)
    if (!item) throw new NotFoundError()
    return item
  }

  public async atualizar(id: number, data: UpdateItemVendaInput): Promise<ItemVenda> {
  ...
  public async excluir(id: number): Promise<{ message: string }> {
    ...
    if (!await deleteItemVenda(id)) throw new NotFoundError()
    return { message: ITEM_VENDA_MESSAGES.DELETED }
  }

  private validarValores(quantidade: number, valorUnitario: number): void {
    if (!Number.isInteger(quantidade) || quantidade < 0) {
      throw new ValidationError(ITEM_VENDA_MESSAGES.INVALID_QUANTITY)
    }
    if (!Number.isFinite(valorUnitario) || valorUnitario < 0) {
      throw new ValidationError(ITEM_VENDA_MESSAGES.INVALID_UNIT_VALUE)
    }
  }
  ...
  private calcularValorTotal(quantidade: number, valorUnitario: number): number {
    ...
    if (!Number.isFinite(total) || total > 99_999_999.99) {
      throw new ValidationError(ITEM_VENDA_MESSAGES.INVALID_TOTAL)
    }
    return total
  }
  ...
  private handleDatabaseError(error: unknown): never {
    ...
    if (databaseError?.code === 'ER_NO_REFERENCED_ROW_2' || databaseError?.errno === 1452) {
      throw new ValidationError(ITEM_VENDA_MESSAGES.INVALID_FOREIGN_KEY)
    }
    throw error
  }
}
