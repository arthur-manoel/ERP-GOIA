...
import {
  ConflictError,
  ItemRequisicaoCompraService,
  NotFoundError,
  ValidationError,
} from './item_requisicao_compra.service.js'
import {
  createItemRequisicaoCompraSchema,
  itemRequisicaoCompraIdSchema,
  listItemRequisicaoCompraSchema,
  updateItemRequisicaoCompraSchema,
} from './item_requisicao_compra.schema.js'

export class ItemRequisicaoCompraController {
  private readonly service = new ItemRequisicaoCompraService()

  public listar = async (req: Request, res: Response): Promise<void> => {
    try {
      const result = await this.service.listar(listItemRequisicaoCompraSchema.parse(req.query))
  ...
  public buscarPorId = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = itemRequisicaoCompraIdSchema.parse(req.params)
  ...
  public criar = async (req: Request, res: Response): Promise<void> => {
    try {
      const data = createItemRequisicaoCompraSchema.parse(req.body)
  ...
  public atualizar = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = itemRequisicaoCompraIdSchema.parse(req.params)
      const data = updateItemRequisicaoCompraSchema.parse(req.body)
  ...
  private handleError(res: Response, error: unknown): void {
    ...
    console.error('[ItemRequisicaoCompraController] Erro inesperado:', error)
